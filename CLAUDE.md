...
the tree, scrubs VCS metadata only from the declared local Harness and Core
replacements, and rejects any remaining embedded `.git` metadata. Never broaden
the scrub to hide undeclared metadata.
